...
V3 = 两个 Agent（检测 + 确认），抓"对照原稿说错"的两类：
  - resay            残句·重说（语义同但措辞异的重说，可删除）
  - report           数值/事实出入（删了句子残破，仅报告、不删、不进确认）
...
### 2. 数值/事实出入 report（仅报告，不删）
口播与原稿在【数字/事实】上不一致，但该内容是句子必需的（删了句子残破、丢信息），无法用"删除"修复 → 归 **report 类型，只报告给用户定夺，不进删除流程、不经确认**：
- `dimension`="misread"，`subtype`="report"
- `delete_text` 留空（不要给删除建议）
- `spoken`=口播实际说的词（如「5500」），`expected`=原稿正确值（如「5000」）
- `error_text`=一句话说明差异与依据（如"口播5500，原稿五千股(5000)，且后文总持仓13000以5000为基准，此处系数值口误"）
- 例：句57 口播「原先剩5500底仓」vs 原稿「五千股底仓」→ report（不删，用户自行决定改/留）
⚠️ report 与 resay 互斥：删了仍通顺、信息由别处承载 → resay（走删除）；删了句子残破/丢信息 → report（只报不删）。**严禁把数值/事实差异包装成 resay 去删**。

## 绝对禁止报告
- ❌ 语气词/口头禅单字及叠读——正常语流，不报
- ❌ 口语化/口头禅短语（咱们/我们/这个/那个/就是说/对吧/是不是/其实/嘛/呀等）：正常语流，**本身不算错**。仅当它在上下文里真构成错误/违和（如口误残留、与后文重说重叠）才报删；自然不违和则不报不删，也不因夹了口语词就误判残句。
- ❌ 主谓宾/语法完整性：短句、无谓语、标题式、过渡句都正常，不要因"读着不完整/无主谓宾"就报
- ❌ 缺它句子就残破的内容词不删：主语/数字/名词（如「原先剩5500底仓」里的数字）即便与原稿数值不同，也不是 resay（不是被后文重说的冗余句），删了句子读不通、丢信息。数值/事实类差异不在 resay 删除范围——resay 只删"被后文重说/半截死路"的冗余句。
...
## 输出格式（严格 JSON）
两类 issue 可出现在同一 issues 数组里：
- resay（可删）：dimension="misread", subtype="resay", 必须给 delete_text
- report（仅报告）：dimension="misread", subtype="report", delete_text 留空，并给 spoken/expected 供报告页展示
{
  "issues": [
    {
      "dimension": "misread", "subtype": "resay",
      "severity": "critical|major|minor",
      "sentence_idx": <句编号>,
      "error_text": "<问题描述>",
      "delete_text": "<整句删填整句原文 / 片段删填精确子串>"
    },
    {
      "dimension": "misread", "subtype": "report",
      "severity": "major",
      "sentence_idx": <句编号>,
      "error_text": "<口播值 vs 原稿值 的差异说明>",
      "spoken": "<口播实际说的词，如 5500>",
      "expected": "<原稿正确值，如 5000>",
      "delete_text": ""
    }
  ]
}
- **没有错误就返回 `{"issues": []}`，绝不强行凑错误**。宁可漏报也不要误报——只有确有把握是真错误时才输出，拿不准的一律不报。
"""
...
- ⚠️ **口语化/口头禅短语不自动判错**：正常语流，**本身不算口误残留**。按上下文是否真错/违和判定——不违和无错 `keep`；确构成错误（如口误残留、被后文重说的残片）才 `delete`。不凭"是口语词"定删留。
- ⚠️ **report 类型不删**：若候选 `subtype` 为 `"report"`（数值/事实出入），它只报告不删除 → 直接 `action="keep"`（或 `action="report"`），`delete_text` 留空，不要删。
...
- ⚠️ **删完读不通就 keep**：若按 `delete_text` 删掉后，候选句变得残破/不通顺/丢失关键信息（如删掉「5500」后「原先剩底仓这会」读不通），说明它不是可删的冗余 resay，而是句子必需的 content 词 → `action="keep"`，`delete_text` 留空。数值/事实差异（如数字与原稿不同）不在 resay 删除范围，不要为"对齐原稿"而删内容词。
